...
form.onsubmit = (e)=>{
    e.preventDefault();//prevenring form from submitting 
    ...
    xhr.onload = ()=>{//once ajax loaded
        if(xhr.readyState == 4 && xhr.status == 200){  //if ajax response status is 200 & ready statuts is 4 means there is no any error
            let response = xhr.response; //storing ajax response in a response variable
            //if response is an error like enter valid email adress then we'll change status color to red else  reset the form 
